...
import { Mail, Send, CheckCircle, AlertCircle, MailCheck } from 'lucide-react'
  ...
  const handleSendEmailChange = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setResult({
        ...
        message: data.message || data.error || 'Erro ao processar mudança de email',
      })
    } catch (error: any) {
      setResult({
        success: false,
            ...
            <GlassCard variant="light" blur="lg" className="p-8">
              <GlassCard.Header className="space-y-2 text-center">
                ...
                <GlassCard.Description className="text-white/90">
                  Teste o processo de mudança de endereço de email
                </GlassCard.Description>
              </GlassCard.Header>

              <GlassCard.Content className="mt-6">
                <form onSubmit={handleSendEmailChange} className="space-y-6">
                  {/* Email */}
                  <div className="space-y-2">
                    ...
                    />
                    <div className="bg-info/20 border border-info/30 rounded-lg p-3 text-xs text-white/90">
                      <p className="font-semibold mb-1">ℹ️ Importante:</p>
                      <p>Mudança de email não pode ser testada via API. Os emails só são enviados quando o usuário está logado e inicia a mudança pela interface do aplicativo.</p>
                    </div>
                  </div>
                  ...
                  >
                    ...
                    {isLoading ? 'Processando...' : 'Verificar Mudança de Email'}
                  </GlassButton>
