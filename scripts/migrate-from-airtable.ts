 ...
 *        - Must have "ID do Paciente" column (ID único - chave primária)
 *        - CPF is optional, extracted from "username" column if available
 *        - CPF is extracted using REGEX_REPLACE({username}, "[^0-9]", "")
 *      - exames.csv (in scripts/data/airtable/)
 *        - Must have "Biologix Exam ID" or "ID Exames" column (chave única do exame)
 *        - Must have "ID Pacientes LINK" column (ID do Paciente - vincula ao paciente)
 *        - Cada exame já vem com o ID do paciente no registro
 *      - tags.csv (in scripts/data/airtable/) - OPTIONAL
 *   2. Configure .env.local with SUPABASE_SERVICE_ROLE_KEY
 *
 * IMPORTANT:
 * - O ID único da tabela pacientes é o "ID do Paciente" (biologix_id), NÃO o CPF
 * - Cada exame já vem com o ID do paciente no registro (ID Pacientes LINK)
 * - CPF é um campo opcional para identificação adicional, mas não é chave única
 */
...
interface AirtablePaciente {
  ...
  'Biologix ID'?: string;
  'ID do Paciente'?: string; // ID único do paciente (chave única - biologix_id)
...
interface AirtableExame {
  'Biologix Exam ID'?: string; // Chave única do exame
  ...
  'ID Pacientes LINK'?: string; // ID do Paciente (biologix_id) - link direto ao paciente
...
async function migrate() {
  ...
  // IMPORTANT: O ID único é o "ID do Paciente" (biologix_id), NÃO o CPF
  ...
  for (const paciente of pacientesData) {
    ...
    const pacienteData: any = {
      biologix_id: biologixPacienteId, // ID do Paciente como chave única (biologix_id)
      nome: paciente.Nome,
    ...
    // IMPORTANT: O ID único é o biologix_id (ID do Paciente), NÃO o CPF
    const { data, error } = await supabase
      .from('pacientes')
      .upsert(pacienteData, {
  ...
  // IMPORTANT: Cada exame já vem com o ID do paciente no registro (ID Pacientes LINK)
